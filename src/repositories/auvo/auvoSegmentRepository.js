...
class AuvoSegmentRepository extends BaseRepository {
  async upsertSegments(segments) {
    const query = `INSERT INTO segments_auvo (segmentId, description) VALUES (?, ?) ON DUPLICATE KEY UPDATE description = VALUES(description), isActive = 1, deletedAt = NULL, lastSyncAt = CURRENT_TIMESTAMP`;
